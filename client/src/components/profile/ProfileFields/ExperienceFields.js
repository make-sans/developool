...
export default class ExperienceFields extends Component {
  ...
  validateExperienceInput = data => {
    ...
    if (data.company.length < 3)
      validatedErrors.company = 'Company name must be at least 3 characters';
    if (data.company.length > 100)
      validatedErrors.company = 'Company name must be max 100 characters';
    if (data.title.length < 3)
      validatedErrors.title = 'Title must be at least 3 characters';
    if (data.title.length > 100)
      validatedErrors.title = 'Title must be max 100 characters';
    if (data.location.length < 3)
      validatedErrors.location = 'Location must be at least 3 characters';
    if (data.location.length > 100)
      validatedErrors.location = 'Location must be max 100 characters';
  ...
  addExperience = e => {
    e.preventDefault();
    const validatedErrors = this.validateExperienceInput(
      this.state.experienceInput
    );
